...
const ApexChart = () => {
  ...
  const getData = async () => {
    try {
      ...
      response.data.forEach((item) => {
        ...
        dadosAgrupadosPorHora[hora] += item.litros_mes.toFixed(2);
      });
      ...
      const dadosParaGrafico = response.data.map((item) => {
        ...
        return {
          ...
          y: parseFloat(item.litros_mes.toFixed(2)), // Consumo de watts por hora com 2 casas decimais
        };
      });
  ...
  const options = {
    ...
    title: {
      text: "Consumo de Litros por Hora",
      align: "left",
      style: {
    ...
    colors: ["#46a8e9"], // Defina cores vibrantes para o gráfico
    fill: {
